...
Enemy::Enemy()
{
	...
	timer_sketch.set_wait_time(EFFECT_SKETCH_INTERVAL);
...
void Enemy::on_update(double delay)
{
	timer_skill.on_update(delay);
	timer_sketch.on_update(delay);
	timer_restore_speed.on_update(delay);

	// calculate position after one move action
	Vector2 move_distance = velocity * delay;
	...
	{
		// select next target block in route
		idx_target++;
	...
	// show animation that matches move direction
	...
	anim_current->on_update(delay);
}

void Enemy::on_render(SDL_Renderer* renderer)
{
	static SDL_Rect health_rect;
	...
	if (hp < max_hp)
	{
		health_rect.x = (int)(position.x - size_hp_bar.x / 2);
		health_rect.y = (int)(position.y - size.y / 2 - size_hp_bar.y - offset_y);
		health_rect.w = (int)(size_hp_bar.x * (hp / max_hp));
		health_rect.h = (int)size_hp_bar.y;
		SDL_SetRenderDrawColor(renderer, color_content.r, color_content.g, color_content.b, color_content.a);
		SDL_RenderFillRect(renderer, &health_rect);

		health_rect.w = (int)size_hp_bar.x;
		SDL_SetRenderDrawColor(renderer, color_border.r, color_border.g, color_border.b, color_border.a);
		SDL_RenderDrawRect(renderer, &health_rect);
	}
}
...
{
	speed = max_speed - 0.5;
	timer_restore_speed.set_wait_time(EFFECT_SLOW_SPEED_INTERVAL);
...
{
	const std::vector<SDL_Point>& idx_list = route->get_idx_list();
